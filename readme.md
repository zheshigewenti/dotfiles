...
su root
cd /etc/
echo 'Server = https://mirrors.tuna.tsinghua.edu.cn/archlinux/$repo/os/$arch' > /etc/pacman.d/mirrorlist # 配置清华源 出现问题可以重置包管理器密钥环pacman-key --init && pacman-key --populate pacman -Sy archlinux-keyring && pacman -Su
